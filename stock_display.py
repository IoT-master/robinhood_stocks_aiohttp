        ...
        while True:
            ...
            for k, v in sorted_stocks:
                ...
                if profit_per_share < 0:
                    print(
                        Fore.RED + f"{k.rjust(8, ' ')}: {aftermarket_price_change:6.2f} [abp: {average_buy_price:8.2f}], [#: {float(v['quantity']):7.2f}] [pps: {profit_per_share:6.2f}], [last: {stats['last']:8.2f}], [bid: {stats['bid']:8.2f}], [ask: {stats['ask']:8.2f}], [real_last: {stats['extended']:8.2f}], [%: {stats['percent']:6.2f}], [dB: {stats['dB']:6.3f}]" + Fore.RESET)
                else:
                    print(
                        Fore.GREEN + f"{k.rjust(8, ' ')}: {aftermarket_price_change:6.2f} [abp: {average_buy_price:8.2f}], [#: {float(v['quantity']):7.2f}] [pps: {profit_per_share:6.2f}], [last: {stats['last']:8.2f}], [bid: {stats['bid']:8.2f}], [ask: {stats['ask']:8.2f}], [real_last: {stats['extended']:8.2f}], [%: {stats['percent']:6.2f}], [dB: {stats['dB']:6.3f}]" + Fore.RESET)
            print(
